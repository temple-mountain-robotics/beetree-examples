...
{   
    ...
    auto dins = platform.discrete_inputs().discrete_input_bank();
    helloworld_model.user_btn_depressed = bte::registry::as<bool>(dins[DiscreteInputValueKey::USER_BTN_DEPRESSED]);
}
...
void Application::actuate() { 
    ...
    platform.discrete_outputs().discrete_output_bank()[DiscreteOutputValueKey::LED] = bte::registry::make(debug_svc.model().is_led_set);
    platform.discrete_outputs().discrete_output_bank()[DiscreteOutputValueKey::DEBUG_PIN] = bte::registry::make(debug_svc.model().is_debug_pin_set);
